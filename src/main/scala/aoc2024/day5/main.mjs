...
const big_input = `XASXMAXXMSXXSMMSXMMSMXSMXMSSMSSSMMSMAMXMXSMMMMAXAMXSASXSSMMSSMXAMXMSAMXMMXAXXXSAMXXXXXMMXSXMXXSMASAMXMXAXXMASAMXXXMAMMMSXSXMXMMMSAASXSMSSMMS
...
const getVerticalXmasCount = (grid) => {
  // stolen property =)
  const rotated = grid[0].map((_, colIndex) =>
...
const getDiagonalDownStartingAt = (grid, row, column) => {
  assert(
    row === 0 || column === 0,
    "row or column is misplaced - one must be 0"
  );
  ...
  const arrayOfExpectedLength = Array.from(Array(expectedIterations)).map(
    (_, index) => index
  );

  const line = arrayOfExpectedLength.map(
    (_, index) => grid[row + index][column + index]
  );
...
const getDiagonalDownXmasCount = (grid) => {
  ...
  const arrayOfRowsLength = Array.from(Array(gridHeight)).map(
    (_, index) => index
  );
  const asdf = arrayOfRowsLength.reduce(
    (acc, row) => acc + getDiagonalDownStartingAt(grid, row, 0),
    0
  );

  // starting at all columns (except the 0)
  const arrayOfColumnsLength = Array.from(Array(gridWidth - 1)).map(
    (_, index) => index
  );
  const fdsa = arrayOfColumnsLength.reduce(
    (acc, col) => acc + getDiagonalDownStartingAt(grid, 0, col + 1),
    0
  );
...
const getGridX_MasCount = (grid) => {
  ...
  for (let row = 1; row < maxRow; row++)
    for (let col = 1; col < maxColumn; col++) {
      ...
      const isXmasCross =
        getLineMasCount(diagonalUp) + getLineMasCount(diagonalDown) === 2;
...
assert(
  diagonalDownResult,
  "something up with diagonal down: " + diagonalDownResult
);
...
assert(
  getLineXmasCount("XMASAMX".split("")) === 2,
  "something up with line count"
);
assert(
